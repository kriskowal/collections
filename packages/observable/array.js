...
var array_swap = require("@collections/swap/array");
...
var observableArrayProperties = {
  ...
  swap: {
    value: function swap(start, minusLength, plus) {
      ...
      if (diff === 0) {
        ...
        for (i = start, j = 0; i < start + plus.length; i++, j++) {
          if (plus[j] !== minus[j]) {
        ...
        for (i = start, j = 0; i < observedLength; i++, j++) {
  ...
  push: {
    value: function push(value) { // push.length === 1
  ...
  unshift: {
    value: function unshift(value) { // unshift.length === 1
