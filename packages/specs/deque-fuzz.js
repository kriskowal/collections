...
function makePlan(length, seed, biasWeight, maxAddLength) {
    ...
    while (ops.length < length) {
        ...
        if (choice1 < 1 / (maxAddLength + 1)) {
            if (choice2 < .5) {
                ops.push(["push", makeRandomArray(1 + ~~(random() * maxAddLength - .5), random)]);
            } else {
                ops.push(["unshift", makeRandomArray(1 + ~~(random() * maxAddLength - .5), random)]);
            }
        } else {
            if (choice2 < .5) {
                ops.push(["shift", []]);
...
function execute(Collection, ops) {
    ...
    ops.forEach(function (op) {
        ...
        if (typeof expect === "function") {
            expect(toArray(actual)).toEqual(oracle);
